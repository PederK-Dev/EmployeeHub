...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // The context's onChange may receive a single date or (for range calendars) an
    // array; this single-date provider only tracks the scalar case.
    const onChange = (next: DateValue | readonly DateValue[]) => setValue(Array.isArray(next) ? (next[0] ?? null) : (next as DateValue));
